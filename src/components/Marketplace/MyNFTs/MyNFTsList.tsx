...
export default function MyNFTsList({ data }: MyNFTsListProps) {
        ...
        <div className={`${data.length ? "grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4" : ""} gap-4 w-full h-fit`}>
            {
                data.length ? data.map((item, index) => {
                    return <MyNFTsCard image={item.metadata.image} name={item.metadata.name} tokenId={item.tokenId} rarity={item.rarity} attributes={item.metadata.attributes} account={item.account} creator={item.creator} category={item.category} className="hover:scale-95 transition delay-150 duration-200 ease-in-out" key={index} contractAddress={
